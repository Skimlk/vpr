...
function rePoints() {
	...
	for(imageCount = 0; imageCount < images.length; imageCount++)
	{
		...
		{
			if(typeof images[imageCount].points[pointCount] !== 'undefined' && typeof images[imageCount].points[pointCount].mapCoords !== "undefined" && images[imageCount].points[pointCount].mapCoords != null)
			{
				...
				addMarker(images[imageCount].points[pointCount].mapCoords, map, iconUrl);
				angle = findAngle(images[imageCount].points[pointCount].imgXCoord);
				addLine(images[imageCount].points[pointCount].mapCoords, angle, strokeColor);
			}
		}
	}
}
...
function addLine(rpoint, deg, color) {
   ...
   line = [
    ...
    { lat: rotatePoint(pnt1, org, -deg).y, lng: rotatePoint(pnt1, org, -deg).x},
   ];
